...
namespace RingVariant
{
    ...
    class TokenRingVariant
    {
        ...
        static void Main(string[] argv)
        {
            ...
            {
                List<NodeVariant> nodes = new List<NodeVariant>();
                nodes.Add(new RingLibrary.MonitorVariant());
                ...
                try
                {
                    using (StreamReader streamy = new StreamReader(argv[0]))
                    {
                        ...
                        while (streamy.Peek() >= 0)
                        {
                            ...
                            nodes.Add(new NodeVariant(i, sock, port));
                            port++;
                        }
                        ...
                        //Now we have to Handle the Monitor.
                        {
                            ...
                            ((RingLibrary.MonitorVariant)nodes[0]).SetMonitor(0, sock, port, Convert.ToUInt16(Convert.ToUInt16(argv[1]) + 1));
                        }
                    }
                }
                catch (Exception)
                ...
                using (StreamWriter writey = new StreamWriter(Bridge.RingOut2))
                {
                    writey.WriteLine(Bridge.ListenPort2); //Tell the bridge where to listen
                ...
                while (true)
                {
                    try
                    {
                        nodes[0].Connect(Bridge.ListenPort2);

                        Console.WriteLine("Ring 2 Connected to Bridge");
                        break;
                    }
                    catch (Exception)
                    {
